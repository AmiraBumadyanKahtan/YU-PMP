...
function createTask($data) {
    ...
    try {
        if ($stmt->execute([
            ...
            if (function_exists('addSystemTodo') && !empty($data['assigned_to'])) {
                addSystemTodo($data['assigned_to'], "New Task: " . substr($data['title'],0,20), "You have a new task.", "task", $taskId);
            }
...
function updateTask($id, $data) {
    $db = Database::getInstance()->pdo();
    $oldData = $db->query("SELECT project_id, milestone_id FROM project_tasks WHERE id=$id")->fetch();
...
// ✅ دالة جديدة: تحديث التقدم والحالة فقط (للموظفين)
...
// دالة مساعدة للتحقق من الصلاحيات (لتجنب الأخطاء إذا لم تكن موجودة في auth.php)
if (!function_exists('userCanInProject')) {
    function userCanInProject($projectId, $permissionKey) {
        $db = Database::getInstance()->pdo();
        // التحقق من الصلاحيات عبر جدول project_user_permissions
        $stmt = $db->prepare("
