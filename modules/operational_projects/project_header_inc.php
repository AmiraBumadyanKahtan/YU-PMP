...
// تأكد أن المتغير $id و $project موجودين
if (!isset($project) || empty($project)) return;

// --- 1. المنطق البرمجي (Calculation Logic) ---
...
// --- 2. التحقق من الصلاحيات للعرض (Visibility Check) ---
// هل المستخدم يملك صلاحية إدارة الصلاحيات؟ (لعرض التاب)
$canManagePerms = userCanInProject($id, 'manage_project_permissions');

// هل المستخدم يملك صلاحية رؤية التعاون؟
$canViewCollab = userCanInProject($id, 'view_project_collaborations');

// هل المستخدم يملك صلاحية رؤية التحديثات؟
$canViewUpdates = userCanInProject($id, 'view_project_updates');

// هل المستخدم يملك صلاحية رؤية الميزانية؟ (لإخفاء الأرقام الحساسة)
...
<style>
    ...
    .ph-code-badge { display: inline-block; background-color: #fff3e0; color: #d35400; padding: 5px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: bold; margin-bottom: 10px; }
    ...
    .stat-label { font-size: 0.8rem; color: #888; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px; }
    .stat-value { font-size: 1.1rem; font-weight: 800; color: #2c3e50; }
    ...
    /* ستايل خاص لتاب الصلاحيات لتمييزه */
    ...
    <div class="ph-stats-bar">
        <div class="stat-item">
            ...
            <div class="stat-info">
                ...
                <div class="stat-value"><?= $project['status_id'] == 5 ? 'Active' : 'Pending' ?></div>
            </div>
        </div>
        ...
        <div class="stat-item">
            ...
            <div class="stat-info">
                ...
                <?php if ($canViewBudget): // عرض الأرقام فقط لمن يملك الصلاحية ?>
                ...
                <?php else: ?>
                    <div class="stat-value" style="font-size:0.9rem; color:#999;"><i>Hidden</i></div>
                <?php endif; ?>
            </div>
        </div>
...
<div class="tabs-container">
    ...
    <?php if ($project['status_id'] == 5): // Approved ?>
